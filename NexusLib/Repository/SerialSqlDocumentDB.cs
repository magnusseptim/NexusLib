...
namespace NexusLib.Repository
{
    public class SerialSqlDocumentDB : ISerialSqlDocumentDB
    {
        ...
        public IList<Task<BaseResponse<Document>>> ReadDocuments(IEnumerable<(string documentID, string partitionKey, string colId, string dbID)> documentParams)
            => documentParams.Select(x => sqlDocumentsDB.ReadDocumentAsync(x.documentID, x.partitionKey, x.colId, x.dbID)).ToList();

        public IList<Task<BaseResponse<Document>>> UpdateDocuments<DocumentType>(IEnumerable<(string documentID, DocumentType document, string colId, string dbID)> documentParams)
            => documentParams.Select(x => sqlDocumentsDB.UpdateDocumentAsync(x.documentID, x.document, x.colId, x.dbID)).ToList();
    }
}
